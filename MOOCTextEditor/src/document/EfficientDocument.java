...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		{
			//System.out.println( "this is a token " +tokens);
			if (isWord(tokens.get(i))) 
			{
				
				numWords ++;
				int current = countSyllables(tokens.get(i));
				numSyllables += current;
				if (i == (tokens.size()-1)) 
				{
					numSentences ++;
				}
			...
			{
				//System.out.println( "Not a Word:" +token);
	...
	{
		System.out.println("Testing EfficientDOcument");
