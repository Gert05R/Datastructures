...
public class NearbyWords implements SpellingSuggest {
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while (retList.size()<numSuggestions && Counter<THRESHOLD && !queue.isEmpty()) 
		{
			...
			List <String> currList = this.distanceOne(curr, false);
			for (int i=0;i<currList.size() && retList.size()<numSuggestions;i++) 
			{
				if( Counter > 0) {
				if (!visited.contains(currList.get(i))) 
				{
					...
					if(dict.isWord(currList.get(i))) 
					{
						retList.add(currList.get(i));
